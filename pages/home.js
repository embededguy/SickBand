...
const Home = ({ route }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response1 = await fetch(`http://${ipAddress}/bpm`);
      ...
      const response2 = await fetch(`http://${ipAddress}/spo2`);
      ...
      const response3 = await fetch(`http://${ipAddress}/ecg`);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Text style={styles.title}>SICK WRIST BAND</Text>
      <Row>
      <Text style={styles.txt}>Temperature ( {temperature} )℃</Text>
      <Text style={styles.txt}>BPM ( {BPM} )</Text>
      </Row>
      <Row>
        <Speedometer value={temperature} size={150} totalValue={50} innerColor="#000" showIndicator showLabels text="50.00" text="Temperature"/>
        <Speedometer value={BPM} size={150} totalValue={200} innerColor="#000" showIndicator showLabels text="50.00" text="Temperature"/>
      </Row>
      <Row>
      </Row>
      <Text style={styles.txt}>SpO2 ( {SpO2}% )</Text>
      <Row>
        <Speedometer value={SpO2} totalValue={120} innerColor="#000" internalColor="#00aaff" showIndicator showLabels text="50.00" text="Temperature"/>
      </Row>
      <Row>
        <LineChart
  data={datax}
